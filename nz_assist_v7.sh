...
kill_sessions_by_username() {
    ...
    AND s.ID != SESSION_ID()  -- Exclude current session
...
custom_session_termination() {
    ...
    # Build WHERE clause dynamically
    local where_conditions=()
    where_conditions+=("s.ID != SESSION_ID()")  # Always exclude current session
    ...
    # Join conditions with AND
    local where_clause="WHERE $(IFS=' AND '; echo "${where_conditions[*]}")"
